...
pub fn length(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
    Ok(StdlibOk::Finished(vm.mem(string + 1)?))
}

pub fn char_at(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
    let pos = params[1];
...
pub fn set_char_at(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
    let pos = params[1];
    let c = params[2];
...
pub fn append_char(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
    let c = params[1];
...
pub fn erase_last_char(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
    let len = vm.mem(string + 1)?;
    if len == 0 {
...
pub fn int_value(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
...
pub fn set_int(vm: &mut VM, _: State, params: &[Word]) -> StdResult {
    let string = params[0] as Address;
...
pub fn backspace(_vm: &mut VM, _: State, _params: &[Word]) -> StdResult {
    Ok(StdlibOk::Finished(BACKSPACE_KEY))
}

pub fn double_quote(_vm: &mut VM, _: State, _params: &[Word]) -> StdResult {
    Ok(StdlibOk::Finished('"' as Word))
}

pub fn newline(_vm: &mut VM, _: State, _params: &[Word]) -> StdResult {
