...
The architecture is divided into three major areas: the actual \verb+simulators+, the \verb+parsers+, and the \verb+interfaces+ that serve as entry points into the application.
...
The \verb+Web UI+, a JavaScript application that calls the emulator implementations as a WebAssembly library, is easily accessible in any standard web browser.
Unlike the other two entry points, which can only use one of the emulators at a time, the \verb+Web UI+ can use both, indicated in the diagram by an arrow pointing to the outer \verb+simulators+ module.
...
Assigning multiple indexes to each instruction increases the index of each opcode in the vector.
...
The exclamation mark in \verb+tos_binary!+ marks this expression as a call to a macro rather than a function, so the \verb*=+= operator can be passed as an argument.
\begin{lstlisting}[
  language=Rust,
...
\begin{lstlisting}[
  ...
  // this struct contains the lexer as a field
